...
function buildIncorrectAnswers(type, correctAnswer, values) {
  if (type === "true_false") {
    return correctAnswer.toLowerCase() === "false" ? "True" : "False";
  }

  if (type === "multiple_choice") {
    const wrongAnswers = uniqueTrimmed(values, correctAnswer);
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const payload = {
      ...
      incorrect_answers: buildIncorrectAnswers(questionType, correctAnswer, body.incorrect_answers),
    ...
    console.error("save-written-question error:", error);
