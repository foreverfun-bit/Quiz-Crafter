...
import { CheckCircle, Loader2, Send, Smartphone, Trophy } from "lucide-react";
...
const getStoredPlayer = (sessionId) => {
  try {
    return JSON.parse(sessionStorage.getItem(`quiz-crafter-player-${sessionId}`) || "null");
  } catch {
    return null;
  }
};
...
const PlayerSession = () => {
      ...
      .subscribe((status) => {
        ...
        if (isConnected) {
          channel.send({
            type: "broadcast",
            event: "player_join",
            payload: { playerId: player.id, playerName: player.name, joinedAt: new Date().toISOString() },
          });
        }
      });
  ...
  const submitAnswer = (value) => {
    ...
    const payload = {
      ...
      answer: finalAnswer,
      questionIndex: hostState.currentIndex,
        ...
        <div className="w-full max-w-sm">
          ...
          <Card className="glass-card">
            <CardContent className="p-5 space-y-4">
              <div>
                <label className="text-zinc-400 text-sm block mb-1.5">Team Name</label>
                <input value={name} onChange={(event) => setName(event.target.value)} onKeyDown={(event) => event.key === "Enter" && joinGame()} placeholder="Enter team name" maxLength={32} className="w-full h-12 rounded-lg bg-zinc-950 border border-white/10 px-3 text-white text-lg outline-none focus:border-[#71E0DC]/60" autoFocus />
              </div>
              <Button onClick={joinGame} className="w-full h-12 gradient-btn text-base font-bold">Join Game</Button>
            </CardContent>
          </Card>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#09090B] text-white flex flex-col" data-testid="player-session-page">
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        ...
        {hostState?.mode === "leaderboard" && (
          <div className="w-full max-w-md">
            <div className="text-center mb-5">
              <Trophy className="mx-auto text-amber-300 mb-2" size={38} />
              <h2 className="text-3xl font-black">Leaderboard</h2>
            </div>
            <div className="space-y-2">
              {leaderboard.map((team, index) => (
                <div key={team.id || team.name} className={`flex items-center justify-between gap-3 rounded-lg border px-4 py-3 ${team.id === player.id ? "border-[#71E0DC]/50 bg-[#71E0DC]/10" : "border-white/10 bg-zinc-950/70"}`}>
        ...
        {hostState && hostState.mode !== "leaderboard" && currentQuestion && (
          <div className="w-full max-w-md">
            <div className="text-center mb-5">
              <Badge className="bg-zinc-800 text-zinc-300 mb-3">{currentQuestion.roundName || "Round"} · {currentQuestion.category}</Badge>
              <h2 className="text-2xl font-black leading-tight">{currentQuestion.questionText}</h2>
            </div>

            {hostState.showAnswer ? (
              <Card className="border-emerald-500/30 bg-emerald-500/10">
                <CardContent className="p-5 text-center">
                  ...
                  {hostState.showFunFact && currentQuestion.funFact && <p className="text-zinc-300 mt-3">{currentQuestion.funFact}</p>}
                </CardContent>
              </Card>
            ) : submitted?.questionIndex === hostState.currentIndex ? (
              <Card className="glass-card">
                <CardContent className="p-6 text-center">
                  <CheckCircle className="mx-auto text-emerald-300 mb-3" size={42} />
                  <h3 className="text-2xl font-black mb-1">Answer Locked</h3>
                  <p className="text-zinc-400">You answered: <span className="text-white font-bold">{submitted.answer}</span></p>
                </CardContent>
              </Card>
            ) : (
              <AnswerForm question={currentQuestion} answer={answer} setAnswer={setAnswer} submitAnswer={submitAnswer} />
            )}
          </div>
        )}
      </main>
...
const AnswerForm = ({ question, answer, setAnswer, submitAnswer }) => {
  if (question.type === "true_false") {
    return (
      <div className="grid grid-cols-2 gap-3">
        <Button onClick={() => submitAnswer("True")} className="h-16 text-xl font-black bg-emerald-500/20 border-2 border-emerald-500/40 text-emerald-300 hover:bg-emerald-500/30">True</Button>
        <Button onClick={() => submitAnswer("False")} className="h-16 text-xl font-black bg-red-500/20 border-2 border-red-500/40 text-red-300 hover:bg-red-500/30">False</Button>
      </div>
    );
  }
  ...
  return (
    <div className="flex gap-2">
      <input value={answer} onChange={(event) => setAnswer(event.target.value)} onKeyDown={(event) => event.key === "Enter" && submitAnswer()} placeholder="Type your answer" className="min-w-0 flex-1 h-14 rounded-lg bg-zinc-950 border border-white/10 px-3 text-white text-lg outline-none focus:border-[#71E0DC]/60" autoFocus />
      <Button onClick={() => submitAnswer()} disabled={!answer.trim()} className="h-14 px-5 gradient-btn"><Send size={20} /></Button>
    </div>
  );
};
