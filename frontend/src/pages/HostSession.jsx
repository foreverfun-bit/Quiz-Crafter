...
const HostSettings = ({ pointsPerQuestion, setPointsPerQuestion, wagerMode, setWagerMode, wagerLimit, setWagerLimit, wagerTiming, setWagerTiming, timerSeconds, setTimerSeconds, timeRemaining, startTimer, resetTimer }) => <Card className="glass-card mb-5"><CardContent className="p-4 lg:p-5"><div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-wide text-zinc-500">Question Setup</p><div className="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-3 xl:min-w-[520px]"><label className="text-xs text-zinc-400">Points<input type="number" min="0" value={pointsPerQuestion} onChange={(event) => setPointsPerQuestion(event.target.value)} className="mt-1 w-full h-11 rounded-md bg-zinc-950 border border-white/10 px-3 text-white outline-none focus:border-[#71E0DC]/60" /></label><label className="text-xs text-zinc-400">Timer seconds<input type="number" min="0" value={timerSeconds} onChange={(event) => setTimerSeconds(event.target.value)} className="mt-1 w-full h-11 rounded-md bg-zinc-950 border border-white/10 px-3 text-white outline-none focus:border-[#71E0DC]/60" /></label><label className="text-xs text-zinc-400">Wager limit<input type="number" min="0" value={wagerLimit} onChange={(event) => { setWagerLimit(event.target.value); setWagerMode(Number(event.target.value || 0) > 0); }} className="mt-1 w-full h-11 rounded-md bg-zinc-950 border border-white/10 px-3 text-white outline-none focus:border-[#71E0DC]/60" /></label></div></div><div className="flex flex-wrap gap-2 xl:justify-end"><Button onClick={() => setWagerTiming((value) => value === "after_answer" ? "before_answer" : "after_answer")} disabled={!wagerMode} className={wagerMode ? "h-11 bg-purple-500/20 text-purple-200 border border-purple-500/30 hover:bg-purple-500/30" : "h-11 bg-zinc-800 text-zinc-500"}>{wagerTiming === "after_answer" ? "Wager After" : "Wager Before"}</Button><Button onClick={startTimer} className="h-11 gradient-btn"><Play size={16} className="mr-2" />Start Timer</Button><Button variant="outline" onClick={resetTimer} className="h-11 border-white/10 text-zinc-300 hover:text-white"><RotateCcw size={16} className="mr-2" />{timeRemaining === null ? "Clear Timer" : `${timeRemaining}s`}</Button></div></div></CardContent></Card>;

const PhonePlayPanel = ({ joinUrl, copyJoinLink, players, answers, leaderboard, gradedAnswers, markAnswer, openScoreModal, pointsPerQuestion, wagerMode, wagerLimit, setMode }) => {
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=132x132&margin=8&data=${encodeURIComponent(joinUrl)}`;
  return <Card className="glass-card"><CardContent className="p-4"><div className="flex items-center justify-between gap-2 mb-4"><div className="flex items-center gap-2 text-white font-semibold"><Users size={18} className="text-[#71E0DC]" />Phone Play</div><Button size="sm" variant="outline" onClick={copyJoinLink} className="h-8 border-white/10 text-zinc-300 hover:text-white"><Copy size={14} className="mr-1" />Copy</Button></div><div className="grid grid-cols-[132px_1fr] gap-4 items-center rounded-lg border border-[#71E0DC]/20 bg-[#71E0DC]/10 p-3 mb-4"><div className="bg-white rounded-md p-1"><img src={qrUrl} alt="Player join QR code" className="w-[124px] h-[124px]" /></div><div className="min-w-0"><div className="flex items-center gap-2 text-[#71E0DC] font-semibold mb-2"><Link size={16} />Player join link</div><p className="text-xs text-zinc-200 break-all">{joinUrl}</p></div></div><div className="grid grid-cols-2 gap-3 mb-4"><div className="rounded-md bg-zinc-950/70 border border-white/10 p-3"><p className="text-xs text-zinc-500">Players</p><p className="text-2xl font-black">{players.length}</p></div><div className="rounded-md bg-zinc-950/70 border border-white/10 p-3"><p className="text-xs text-zinc-500">Answers</p><p className="text-2xl font-black">{answers.length}</p></div></div><Button size="sm" onClick={() => setMode("leaderboard")} className="w-full mb-4 bg-zinc-800 hover:bg-zinc-700 text-zinc-100"><Trophy size={14} className="mr-2" />Show Leaderboard</Button><div className="mb-3 flex items-center justify-between gap-2"><p className="text-xs font-bold uppercase tracking-wide text-zinc-500">Answers This Question</p><p className="text-[11px] text-zinc-600">Click answer = correct</p></div><div className="space-y-3 max-h-[460px] overflow-y-auto pr-1">{answers.map((answer) => { const graded = gradedAnswers[answerKey(answer)]; const previousPoints = Number(graded?.points || 0); const award = wagerMode && Number(answer.wagerAmount) > 0 ? getWagerAward(answer, leaderboard, wagerLimit, previousPoints) : pointsPerQuestion; const score = getTeamScore(leaderboard, answer.playerId); return <div key={`${answer.playerId}-${answer.questionIndex}`} className={`rounded-md border p-3 transition ${graded?.status === "correct" ? "border-emerald-500/40 bg-emerald-500/10" : graded?.status === "incorrect" ? "border-red-500/40 bg-red-500/10" : "border-white/10 bg-zinc-950/60"}`}><div className="flex items-center justify-between gap-2 mb-2"><div className="min-w-0"><span className="font-semibold text-sm truncate block">{answer.playerName}</span><span className="text-[11px] text-zinc-500">{Number(score)} pts{answer.wagerMode ? ` · Wager ${answer.wagerAmount}${Number(answer.wagerCap || 0) ? ` / ${answer.wagerCap}` : ""}` : ""}</span></div><div className="flex items-center gap-1"><Button size="sm" variant="outline" onClick={() => openScoreModal(answer.playerId, { playerName: answer.playerName, answer })} className="h-7 border-white/10 px-2 text-zinc-300 hover:text-white">Adjust</Button><Button size="sm" onClick={() => markAnswer(answer, "incorrect")} className="h-7 w-7 p-0 bg-red-500/15 text-red-300 hover:bg-red-500/25" aria-label="Mark incorrect"><XCircle size={15} /></Button></div></div><button type="button" onClick={() => markAnswer(answer, "correct")} className="w-full rounded-md border border-white/10 bg-zinc-900/70 px-3 py-2.5 text-left text-sm text-zinc-100 hover:border-emerald-500/50 hover:bg-emerald-500/10"><span className="break-words">{answer.answer}</span></button><div className="mt-2 flex items-center justify-between text-[11px]"><span className={graded?.status === "correct" ? "text-emerald-300" : graded?.status === "incorrect" ? "text-red-300" : "text-zinc-600"}>{graded?.status ? `${graded.status}${Number(graded.points || 0) > 0 ? ` +${graded.points}` : Number(graded.points || 0) < 0 ? ` ${graded.points}` : ""}` : "Ungraded"}</span><span className="text-zinc-600">Correct: +{award}</span></div></div>; })}{!answers.length && <p className="text-xs text-zinc-500 text-center py-4">Answers for the current question will appear here.</p>}</div></CardContent></Card>;
};
...
const QuestionStage = ({ question, index, total, roundName, showAnswer, showFunFact, focusMode, pointsPerQuestion, timeRemaining, wagerMode, wagerLimit, wagerTiming, branding }) => {
