...
const normalizeBranding = (branding = {}) => {
  ...
  return {
    ...
    pausedMessage: String(source.pausedMessage || "").trim() || DEFAULT_BRANDING.pausedMessage,
  };
};
...
const HostSession = () => {
  ...
  // submission on phones, which don't gate on this at all. Restored from
  // hosted_results.liveState on load so it survives a host page reload.
  ...
  const [attendance, setAttendance] = useState({ barHeadcount: "", teamHeadcounts: {} });
  ...
  useEffect(() => {
    const loadSession = async () => {
      ...
      try {
        ...
        // A session-wide setting, not tied to any one question -- restore it
        ...
        setAttendance({
          barHeadcount: data?.attendance?.barHeadcount ?? "",
          teamHeadcounts: data?.attendance?.teamHeadcounts && typeof data.attendance.teamHeadcounts === "object" ? data.attendance.teamHeadcounts : {},
        });
  ...
  // Same isTestRun guard as question settings -- a Test Run's headcounts
  // shouldn't overwrite the real session's attendance numbers.
  useEffect(() => {
    if (!attendanceLoadedRef.current || isTestRun) return undefined;
    const timeout = window.setTimeout(() => {
      supabase.from("sessions").update({ attendance }).eq("id", id).then(({ error }) => {
        if (error) console.warn("Attendance save unavailable:", error);
      }).catch((error) => console.warn("Attendance save unavailable:", error));
    }, 600);
  ...
  // Dropped teams keep their score and history (unlike Remove) but stop
  // counting toward "waiting on" and submitted totals for future questions.
  const toggleTeamDropped = (teamId) => {
    setLeaderboard((teams) => teams.map((team) => team.id === teamId ? { ...team, dropped: !team.dropped } : team));
  };
  const updateBarHeadcount = (value) => setAttendance((current) => ({ ...current, barHeadcount: value }));
  const updateTeamHeadcount = (teamId, value) => setAttendance((current) => ({ ...current, teamHeadcounts: { ...current.teamHeadcounts, [teamId]: value } }));
  ...
  const updateQuestionSettings = async (patch, scope, baseQuestion) => {
    ...
    // A Test Run promises nothing here counts -- points/timer/wager edits
    // made during one must stay local to this run, or they'd overwrite the
    // real question data and show up when the actual event starts.
    if (isTestRun) return;
    try {
  ...
  // Dropped teams stay in activePlayers (history, answer review) but drop
  // out of the "still playing" roster used for waiting-on/submitted counts
  // and the manual-answer dropdown.
  ...
  return <div className="min-h-screen bg-[#09090B] text-white" data-testid="host-session-page" style={brandStyle}>
    ...
    {!focusMode && <ToolDrawer activeDrawer={activeDrawer} onClose={() => setActiveDrawer(null)}>
      {activeDrawer === "teams" && <LeaderboardPanel leaderboard={leaderboard} teamName={teamName} teamScore={teamScore} setTeamName={setTeamName} setTeamScore={setTeamScore} addTeam={addTeam} openScoreModal={openScoreModal} removeTeam={removeTeam} toggleTeamDropped={toggleTeamDropped} showLeaderboard={() => releaseMode("leaderboard")} fairPlayStats={fairPlayStats} attendance={attendance} updateBarHeadcount={updateBarHeadcount} updateTeamHeadcount={updateTeamHeadcount} />}
...
const LiveSignalsPanel = ({ feedback, categoryFeedback, ideas, currentIndex, displayedQuestion }) => {
  ...
  const categoryRows = Object.values(categoryFeedback.reduce((groups, item) => {
    ...
    if (!groups[label]) groups[label] = { key: `category-${label}`, label, likes: 0, dislikes: 0, likedBy: [], dislikedBy: [] };
    const teamName = item.playerName || "Team";
    if (item.sentiment === "like") { groups[label].likes += 1; groups[label].likedBy.push(teamName); }
    if (item.sentiment === "dislike") { groups[label].dislikes += 1; groups[label].dislikedBy.push(teamName); }
    return groups;
...
const LeaderboardPanel = ({ leaderboard, teamName, teamScore, setTeamName, setTeamScore, addTeam, openScoreModal, removeTeam, toggleTeamDropped, showLeaderboard, fairPlayStats, attendance, updateBarHeadcount, updateTeamHeadcount }) => { const sorted = [...leaderboard].sort((a, b) => Number(b.score || 0) - Number(a.score || 0)); const teamHeadcounts = attendance?.teamHeadcounts || {}; return <Card className="glass-card"><CardContent className="p-3"><div className="flex items-center justify-between gap-2 mb-3"><div className="flex items-center gap-2 text-white font-semibold"><Trophy size={18} className="text-amber-300" />Leaderboard</div><Button size="sm" variant="outline" onClick={showLeaderboard} className="h-8 border-white/10 text-zinc-300 hover:text-white">Show</Button></div><div className="mb-3 flex items-center justify-between gap-2 rounded-md border border-white/10 bg-zinc-950/60 px-3 py-2"><span className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-zinc-500"><Users size={13} />Bar headcount</span><input type="number" min="0" value={attendance?.barHeadcount ?? ""} onChange={(event) => updateBarHeadcount(event.target.value)} placeholder="e.g. 60" className="h-8 w-20 rounded-md border border-white/10 bg-zinc-900 px-2 text-right text-sm text-white outline-none focus:border-[#71E0DC]/60" /></div><div className="grid grid-cols-[1fr_76px_36px] gap-2 mb-3"><input value={teamName} onChange={(event) => setTeamName(event.target.value)} onKeyDown={(event) => event.key === "Enter" && addTeam()} placeholder="Team name" className="h-9 rounded-md bg-zinc-950 border border-white/10 px-3 text-sm text-white outline-none focus:border-[#71E0DC]/60" /><input value={teamScore} onChange={(event) => setTeamScore(event.target.value)} onKeyDown={(event) => event.key === "Enter" && addTeam()} placeholder="Score" type="number" className="h-9 rounded-md bg-zinc-950 border border-white/10 px-2 text-sm text-white outline-none focus:border-[#71E0DC]/60" /><Button onClick={addTeam} className="h-9 w-9 p-0 gradient-btn" aria-label="Add team"><Plus size={16} /></Button></div><div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">{sorted.map((team) => { const stats = fairPlayStats.get(team.id) || {}; return <div key={team.id} className={`rounded-md border border-white/10 bg-zinc-950/60 p-2 ${team.dropped ? "opacity-60" : ""}`}><div className="flex items-center justify-between gap-2 mb-2"><span className="flex min-w-0 items-center gap-1.5"><span className="font-semibold text-sm truncate">{team.name}</span>{team.dropped && <span className="shrink-0 rounded-full bg-zinc-800 px-1.5 py-0.5 text-[10px] font-bold text-zinc-400">Dropped</span>}</span><span className="flex shrink-0 items-center gap-2"><input type="number" min="0" value={teamHeadcounts[team.id] ?? ""} onChange={(event) => updateTeamHeadcount(team.id, event.target.value)} placeholder="#" title="People on this team" className="h-6 w-12 rounded border border-white/10 bg-zinc-900 px-1 text-center text-xs text-zinc-300 outline-none focus:border-[#71E0DC]/60" /><span className="font-black text-[#71E0DC]">{Number(team.score || 0)}</span></span></div>{Boolean(stats.flags?.length) && <div className="mb-2 flex flex-wrap gap-1">{stats.flags.map((flag) => <span key={flag} className="rounded-full bg-red-400/15 px-2 py-0.5 text-[11px] font-bold text-red-200">{flag}</span>)}</div>}<div className="mb-2 grid grid-cols-3 gap-1 text-[11px] text-zinc-500"><span>Streak {stats.correctStreak || 0}</span><span>Late {stats.lateCorrect || 0}</span><span>Exit {stats.leftScreen || 0}</span></div><div className="flex items-center justify-end gap-1"><Button size="sm" variant="outline" onClick={() => openScoreModal(team.id)} className="h-7 min-w-24 border-white/10 text-zinc-300 hover:text-white">Edit</Button><Button size="sm" variant="outline" onClick={() => toggleTeamDropped(team.id)} className="h-7 w-8 p-0 border-white/10 text-zinc-400 hover:text-amber-300" aria-label={team.dropped ? "Restore team" : "Mark team dropped"} title={team.dropped ? "Restore team" : "Mark team dropped"}>{team.dropped ? <RotateCcw size={13} /> : <UserX size={13} />}</Button><Button size="sm" variant="outline" onClick={() => removeTeam(team.id)} className="h-7 w-8 p-0 border-white/10 text-zinc-400 hover:text-red-300" aria-label="Remove team"><Trash2 size={13} /></Button></div></div>; })}{!sorted.length && <p className="text-xs text-zinc-500 text-center py-3">Teams will appear here when players join from their phones.</p>}</div></CardContent></Card>; };
