...
const AnswerRows = ({ question, players, answers, activity, fairPlayStats, gradedAnswers, markAnswer, addManualAnswer, wagerMode, wagerLimit, pointsPerQuestion, setMode, isReviewing }) => {
  ...
  return <div>
    <div className="grid gap-2 mb-3">
      {rows.map((row) => {
        ...
        const tone = !hasAnswers ? "neutral" : allCorrect ? "correct" : anyIncorrect ? "incorrect" : "neutral";
        const wagerTotal = row.answers.filter(hasSubmittedWager).reduce((sum, answer) => sum + Number(answer.wagerAmount || 0), 0);
        return <button key={row.key} type="button" disabled={!hasAnswers || isReviewing} onClick={() => toggleRow(row)} className={`flex w-full items-center gap-2.5 rounded-lg border px-3 py-2.5 text-left text-sm transition disabled:cursor-default ${optionToneClasses[tone]} ${!hasAnswers ? "opacity-60" : ""}`}>
