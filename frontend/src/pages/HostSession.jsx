...
const HostSession = () => {
  ...
  const markAnswer = (answer, status) => {
    ...
    const award = wagerMode && Number(answer.wagerAmount) > 0 ? Number(answer.wagerAmount) : Number(pointsPerQuestion) || getDefaultPoints(currentQuestion);
    const previous = gradedAnswers[key];
    const previousPoints = previous?.status === "correct" ? Number(previous.points || 0) : 0;
    const nextPoints = status === "correct" ? award : 0;
  ...
  return <div className="min-h-screen bg-[#09090B] text-white" data-testid="host-session-page">
    ...
    <div className={`max-w-7xl mx-auto p-4 lg:p-6 ${focusMode ? "min-h-screen flex flex-col" : ""}`}>
      {focusMode && <div className="flex justify-between items-center mb-4"><Badge className="bg-zinc-800 text-zinc-300">{currentRound?.name || "Round"} Â· {currentIndex + 1} / {questions.length}</Badge><Button variant="outline" onClick={() => setFocusMode(false)} className="border-white/10 text-zinc-300 hover:text-white">Exit Focus</Button></div>}
