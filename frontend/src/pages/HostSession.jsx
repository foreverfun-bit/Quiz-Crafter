...
const PhonePlayPanel = ({ joinUrl, copyJoinLink, players, answers, gradedAnswers, markAnswer, pointsPerQuestion, setMode }) => {
  ...
  return <Card className="glass-card"><CardContent className="p-4"><div className="flex items-center justify-between gap-2 mb-4"><div className="flex items-center gap-2 text-white font-semibold"><Users size={18} className="text-[#71E0DC]" />Answers Submitted ({submittedCount}/{playerCount || 0})</div><Button size="sm" variant="outline" onClick={copyJoinLink} className="h-8 border-white/10 text-zinc-300 hover:text-white"><Copy size={14} className="mr-1" />Copy Link</Button></div><div className={`rounded-lg border p-3 mb-4 ${allSubmitted ? "border-emerald-500/30 bg-emerald-500/10" : "border-[#71E0DC]/20 bg-[#71E0DC]/10"}`}><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className={`text-sm font-bold ${allSubmitted ? "text-emerald-300" : "text-[#71E0DC]"}`}>{allSubmitted ? "All answers are in" : "Waiting on answers"}</p><p className="mt-1 text-xs text-zinc-300 break-all">{joinUrl}</p></div><Badge className={allSubmitted ? "bg-emerald-500/15 text-emerald-300 border border-emerald-500/20" : "bg-zinc-800 text-zinc-300"}>{submittedCount}/{playerCount || 0}</Badge></div></div><Button size="sm" onClick={() => setMode("leaderboard")} className="w-full mb-4 bg-zinc-800 hover:bg-zinc-700 text-zinc-100"><Trophy size={14} className="mr-2" />Show Leaderboard</Button><div className="mb-3 flex items-center justify-between gap-2"><p className="text-xs font-bold uppercase tracking-wide text-zinc-500">Click an answer to toggle true/false</p><p className="text-[11px] text-zinc-600">Correct: +{pointsPerQuestion}</p></div><div className="flex flex-wrap gap-3 max-h-[520px] overflow-y-auto pr-1">{groups.map((group) => { const allCorrect = group.answers.every((answer) => gradedAnswers[answerKey(answer)]?.status === "correct"); const anyIncorrect = group.answers.some((answer) => gradedAnswers[answerKey(answer)]?.status === "incorrect"); const tone = allCorrect ? "correct" : anyIncorrect ? "incorrect" : "ungraded"; const className = tone === "correct" ? "border-emerald-500/35 bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/25" : tone === "incorrect" ? "border-rose-500/35 bg-rose-500/20 text-rose-300 hover:bg-rose-500/25" : "border-white/10 bg-zinc-900/80 text-zinc-200 hover:border-[#71E0DC]/40"; return <button key={group.key} type="button" onClick={() => toggleGroup(group)} className={`inline-flex max-w-full items-center gap-2 rounded-xl border px-3 py-2 transition ${className}`}><span className="flex -space-x-2">{group.answers.slice(0, 4).map((answer) => <span key={answer.playerId} title={answer.playerName} className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-zinc-950 bg-zinc-200 text-xs font-black text-zinc-800">{String(answer.playerName || "?").slice(0, 1).toUpperCase()}</span>)}{group.answers.length > 4 && <span className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-zinc-950 bg-zinc-700 text-xs font-black text-white">+{group.answers.length - 4}</span>}</span><span className="min-w-0 max-w-[150px] truncate text-sm font-bold">{group.label}</span><span className="text-xs font-bold">{tone === "correct" ? "True" : tone === "incorrect" ? "False" : `${group.answers.length}`}</span>{tone === "correct" ? <CheckCircle size={15} /> : tone === "incorrect" ? <XCircle size={15} /> : null}</button>; })}{!groups.length && <p className="w-full text-xs text-zinc-500 text-center py-4">Answers for the current question will appear here.</p>}</div></CardContent></Card>;
};
