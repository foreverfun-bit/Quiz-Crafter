...
const HostSession = () => {
  ...
  }, [id, session, sessionName, questions.length, currentQuestion, currentIndex, showAnswer, showFunFact, presentMode, leaderboard, players, pointsPerQuestion, timerEndAt, timeRemaining, acceptingAnswers]);
      ...
      <div className={`max-w-7xl mx-auto p-4 lg:p-6 ${focusMode ? "min-h-screen flex flex-col" : ""}`}>
        ...
        <div className={focusMode ? "flex-1 flex items-center" : "grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-5"}>
          <main className="w-full">
            ...
            <HostSettings pointsPerQuestion={pointsPerQuestion} setPointsPerQuestion={setPointsPerQuestion} timerSeconds={timerSeconds} setTimerSeconds={setTimerSeconds} timeRemaining={timeRemaining} startTimer={startTimer} resetTimer={resetTimer} />
            <QuestionStage question={currentQuestion} index={currentIndex} total={questions.length} roundName={currentRound?.name} showAnswer={showAnswer} showFunFact={showFunFact} focusMode={focusMode} pointsPerQuestion={pointsPerQuestion} timeRemaining={timeRemaining} />
...
const HostSettings = ({ pointsPerQuestion, setPointsPerQuestion, timerSeconds, setTimerSeconds, timeRemaining, startTimer, resetTimer }) => <Card className="glass-card mb-4"><CardContent className="p-3 grid grid-cols-1 md:grid-cols-[1fr_1fr_auto_auto] gap-2 items-end"><label className="text-xs text-zinc-400">Points<input type="number" min="0" value={pointsPerQuestion} onChange={(event) => setPointsPerQuestion(event.target.value)} className="mt-1 w-full h-9 rounded-md bg-zinc-950 border border-white/10 px-3 text-white outline-none focus:border-[#71E0DC]/60" /></label><label className="text-xs text-zinc-400">Timer seconds<input type="number" min="5" value={timerSeconds} onChange={(event) => setTimerSeconds(event.target.value)} className="mt-1 w-full h-9 rounded-md bg-zinc-950 border border-white/10 px-3 text-white outline-none focus:border-[#71E0DC]/60" /></label><Button onClick={startTimer} className="h-9 gradient-btn"><Play size={15} className="mr-2" />Start Timer</Button><Button variant="outline" onClick={resetTimer} className="h-9 border-white/10 text-zinc-300 hover:text-white"><RotateCcw size={15} className="mr-2" />{timeRemaining === null ? "Clear" : `${timeRemaining}s`}</Button></CardContent></Card>;

const PhonePlayPanel = ({ joinUrl, copyJoinLink, players, answers, adjustScore, pointsPerQuestion, setMode }) => <Card className="glass-card"><CardContent className="p-3"><div className="flex items-center justify-between gap-2 mb-3"><div className="flex items-center gap-2 text-white font-semibold"><Users size={18} className="text-[#71E0DC]" />Phone Play</div><Button size="sm" variant="outline" onClick={copyJoinLink} className="h-8 border-white/10 text-zinc-300 hover:text-white"><Copy size={14} className="mr-1" />Copy</Button></div><div className="rounded-lg border border-[#71E0DC]/20 bg-[#71E0DC]/10 p-3 mb-3"><div className="flex items-center gap-2 text-[#71E0DC] font-semibold mb-2"><Link size={16} />Player join link</div><p className="text-xs text-zinc-200 break-all">{joinUrl}</p></div><div className="grid grid-cols-2 gap-2 mb-3"><div className="rounded-md bg-zinc-950/70 border border-white/10 p-2"><p className="text-xs text-zinc-500">Players</p><p className="text-xl font-black">{players.length}</p></div><div className="rounded-md bg-zinc-950/70 border border-white/10 p-2"><p className="text-xs text-zinc-500">Answers</p><p className="text-xl font-black">{answers.length}</p></div></div><Button size="sm" onClick={() => setMode("leaderboard")} className="w-full mb-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-100"><Trophy size={14} className="mr-2" />Show Leaderboard</Button><div className="space-y-2 max-h-48 overflow-y-auto pr-1">{answers.map((answer) => <div key={`${answer.playerId}-${answer.questionIndex}`} className="rounded-md border border-white/10 bg-zinc-950/60 p-2"><div className="flex items-center justify-between gap-2 mb-1"><span className="font-semibold text-sm truncate">{answer.playerName}</span><Button size="sm" onClick={() => adjustScore(answer.playerId, pointsPerQuestion)} className="h-7 bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30">+{pointsPerQuestion}</Button></div><p className="text-sm text-zinc-300 break-words">{answer.answer}</p></div>)}{!answers.length && <p className="text-xs text-zinc-500 text-center py-3">Answers for the current question will appear here.</p>}</div></CardContent></Card>;
