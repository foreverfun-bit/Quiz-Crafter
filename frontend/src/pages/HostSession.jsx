  ...
  return <div className="min-h-screen bg-[#09090B] text-white" data-testid="host-session-page" style={brandStyle}>
    ...
    <div className={`max-w-[1680px] mx-auto p-4 lg:p-8 ${focusMode ? "min-h-screen flex flex-col" : ""}`}>
      ...
      <div className={focusMode ? "flex-1 flex items-center" : "grid grid-cols-1 2xl:grid-cols-[minmax(0,1fr)_430px] gap-7"}>
        ...
        {!focusMode && <HostSidePanel activeTab={sidePanelTab} setActiveTab={setSidePanelTab} joinUrl={joinUrl} copyJoinLink={copyJoinLink} players={players} answers={answers} currentAnswers={currentAnswers} leaderboard={leaderboard} gradedAnswers={gradedAnswers} markAnswer={markAnswer} openScoreModal={openScoreModal} pointsPerQuestion={Number(pointsPerQuestion) || getDefaultPoints(currentQuestion)} wagerMode={wagerMode} wagerLimit={wagerLimit} setMode={releaseMode} teamName={teamName} teamScore={teamScore} setTeamName={setTeamName} setTeamScore={setTeamScore} addTeam={addTeam} adjustScore={adjustScore} removeTeam={removeTeam} showLeaderboard={() => releaseMode("leaderboard")} rounds={rounds} currentIndex={currentIndex} goToQuestion={goToQuestion} editBuild={() => navigate(`/build/${id}`)} />}
      </div>
    </div>
...
const HostSidePanel = ({ activeTab, setActiveTab, joinUrl, copyJoinLink, players, answers, currentAnswers, leaderboard, gradedAnswers, markAnswer, openScoreModal, pointsPerQuestion, wagerMode, wagerLimit, setMode, teamName, teamScore, setTeamName, setTeamScore, addTeam, adjustScore, removeTeam, showLeaderboard, rounds, currentIndex, goToQuestion, editBuild }) => {
  const tabs = [
    { key: "answers", label: "Answers", icon: MessageSquare },
    { key: "teams", label: "Teams", icon: Trophy },
    { key: "run", label: "Run Sheet", icon: MonitorPlay },
  ];
  return <aside className="space-y-4"><Card className="glass-card"><CardContent className="p-2"><div className="grid grid-cols-3 gap-1">{tabs.map(({ key, label, icon: Icon }) => <button key={key} type="button" onClick={() => setActiveTab(key)} className={`h-10 rounded-md text-xs font-bold flex items-center justify-center gap-1.5 transition ${activeTab === key ? "text-zinc-950" : "text-zinc-300 hover:bg-white/5 hover:text-white"}`} style={activeTab === key ? { background: "linear-gradient(90deg, var(--host-primary), var(--host-accent))" } : undefined}><Icon size={14} />{label}</button>)}</div></CardContent></Card>{activeTab === "answers" && <PhonePlayPanel joinUrl={joinUrl} copyJoinLink={copyJoinLink} players={players} answers={currentAnswers} leaderboard={leaderboard} gradedAnswers={gradedAnswers} markAnswer={markAnswer} openScoreModal={openScoreModal} pointsPerQuestion={pointsPerQuestion} wagerMode={wagerMode} wagerLimit={wagerLimit} setMode={setMode} />}{activeTab === "teams" && <LeaderboardPanel leaderboard={leaderboard} teamName={teamName} teamScore={teamScore} setTeamName={setTeamName} setTeamScore={setTeamScore} addTeam={addTeam} adjustScore={adjustScore} openScoreModal={openScoreModal} removeTeam={removeTeam} showLeaderboard={showLeaderboard} />}{activeTab === "run" && <RunSheet rounds={rounds} currentIndex={currentIndex} goToQuestion={goToQuestion} answers={answers} players={players} gradedAnswers={gradedAnswers} editBuild={editBuild} />}</aside>;
};
