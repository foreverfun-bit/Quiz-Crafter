    ...
    <div className={`max-w-[1680px] mx-auto p-4 lg:p-8 ${focusMode ? "min-h-screen flex flex-col" : ""}`}>
      ...
      <div className={focusMode ? "flex-1 flex items-center" : "grid grid-cols-1 2xl:grid-cols-[minmax(0,1fr)_430px] gap-7"}>
        <main className="w-full">
          ...
          <HostProgressControls hostIndex={hostIndex} currentIndex={currentIndex} total={questions.length} isReviewing={isReviewing} presentMode={presentMode} showAnswer={showAnswer} hasRevealExtra={hasRevealExtra} hasFunFact={Boolean(displayedQuestion.funFact)} pendingBonusIndex={pendingBonusIndex} onPrevious={() => reviewQuestion(hostIndex - 1)} onToggleAnswer={toggleAnswer} onToggleFunFact={toggleFunFact} onReviewNext={() => hostIndex < currentIndex ? reviewQuestion(hostIndex + 1) : returnToLiveQuestion()} onNext={() => presentMode === "bonus_pause" && pendingBonusIndex !== null ? goToQuestion(pendingBonusIndex, { startTimer: true, pauseBeforeBonus: false }) : goToQuestion(currentIndex + 1, { startTimer: true })} />
