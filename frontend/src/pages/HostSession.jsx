...
const buildFairPlayStats = (players, answers, gradedAnswers) => {
  ...
  const byTeam = new Map([...teamIds].map((teamId) => [teamId, { lateCorrect: 0, correctStreak: 0, longestCorrectStreak: 0, correct: 0, graded: 0, flags: [] }]));
...
const HostSession = () => {
  ...
  useEffect(() => {
    const loadSession = async () => {
      ...
      try {
        ...
        setAttendance({
          nonPlayers: normalizeHeadcount(data?.attendance?.nonPlayers ?? data?.attendance?.barHeadcount),
  ...
  const fairPlayStats = useMemo(() => buildFairPlayStats(players, answers, gradedAnswers), [players, answers, gradedAnswers]);
  ...
  return <div className="min-h-screen bg-[#09090B] text-white" data-testid="host-session-page" style={brandStyle}>
    ...
    <div className={`max-w-[1680px] mx-auto p-4 lg:p-8 ${focusMode ? "min-h-screen flex flex-col" : ""}`}>
      ...
      <div className={focusMode ? "flex-1 flex items-center" : "grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_360px] gap-7"}>
        ...
        {!focusMode && <PlaybackRail mode={presentMode} setMode={releaseMode} rounds={rounds} introRoundKey={selectedIntroKey} chooseIntroRound={chooseIntroRound} isReviewing={isReviewing} showAnswer={isReviewing ? true : showAnswer} onRevealAnswer={toggleAnswer} showFunFact={isReviewing ? false : showFunFact} onShowFunFact={toggleFunFact} hasRevealExtra={hasRevealExtra} hasFunFact={Boolean(displayedQuestion.funFact)} funFact={displayedQuestion.funFact} hasAudio={Boolean(displayedQuestion.audio_url)} isPlayingAudio={isPlayingAudio} onToggleAudio={toggleAudioPlayback} timeRemaining={isReviewing ? null : timeRemaining} timerSeconds={viewTimerSeconds} startTimer={startTimer} resetTimer={isReviewing ? () => {} : resetTimer} currentIndex={currentIndex} total={questions.length} onPrev={() => goToQuestion(currentIndex - 1, { startTimer: false })} onNext={() => {
    ...
    <Card className="glass-card">
      <CardContent className="p-3.5">
        <div className="mb-2.5 flex items-center justify-between gap-2">
          <p className="text-xs font-bold uppercase tracking-wide text-zinc-500">Display Controls</p>
...
const FairPlayPanel = ({ leaderboard, fairPlayStats, openScoreModal }) => {
  const flagged = leaderboard
    ...
    .filter(({ stats }) => (stats.flags?.length || 0) > 0 || Number(stats.lateCorrect || 0) > 0 || Number(stats.correctStreak || 0) >= 3);
  return <Card className="glass-card"><CardContent className="p-4 space-y-4"><div><div className="flex items-center gap-2 text-white font-semibold"><AlertTriangle size={18} className="text-amber-300" />Fair Play</div><p className="text-xs text-zinc-500 mt-1">Streaks and late-correct answers worth a second look.</p></div><div className="space-y-2 max-h-[560px] overflow-y-auto pr-1">{flagged.map(({ team, stats }) => <div key={team.id} className="rounded-lg border border-white/10 bg-zinc-950/60 p-3"><div className="mb-2 flex items-center justify-between gap-2"><span className="font-semibold text-sm truncate">{team.name}</span><span className="font-black text-[#71E0DC]">{Number(team.score || 0)}</span></div>{Boolean(stats.flags?.length) && <div className="mb-2 flex flex-wrap gap-1">{stats.flags.map((flag) => <span key={flag} className="rounded-full bg-red-400/15 px-2 py-0.5 text-[11px] font-bold text-red-200">{flag}</span>)}</div>}<div className="mb-2 grid grid-cols-2 gap-1 text-[11px] text-zinc-500"><span>Streak {stats.correctStreak || 0}</span><span>Late {stats.lateCorrect || 0}</span></div><Button size="sm" variant="outline" onClick={() => openScoreModal(team.id)} className="h-7 border-white/10 text-zinc-300 hover:text-white">Adjust Score</Button></div>)}{!flagged.length && <p className="text-xs text-zinc-500 text-center py-6">No fair-play flags yet. Streaks and late-correct answers will show up here as the game runs.</p>}</div></CardContent></Card>;
};
...
const LeaderboardPanel = ({ leaderboard, teamName, teamScore, setTeamName, setTeamScore, addTeam, openScoreModal, removeTeam, toggleTeamDropped, showLeaderboard, fairPlayStats, attendance, updateNonPlayers, updateTeamHeadcount }) => {
