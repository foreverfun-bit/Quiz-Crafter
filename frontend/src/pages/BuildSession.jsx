...
const BuildSession = () => {
  ...
  const fullSessionTypePlan = (needed, templateQuestionType = "mixed") => { const plan = { multiple_choice: 0, true_false: 0, written: 0 }; if (templateQuestionType && templateQuestionType !== "mixed" && plan[templateQuestionType] !== undefined) { plan[templateQuestionType] = needed; return plan; } const order = ["multiple_choice", "true_false", "written", "multiple_choice", "true_false"]; for (let index = 0; index < needed; index += 1) plan[order[index % order.length]] += 1; return plan; };
  ...
  const requestGeneratedForFullRound = async ({ round, type, count, excludedCategories, generatedQuestions }) => { const latestMemory = readQuestionMemory(); setQuestionMemory(latestMemory); const rejectedQuestions = new Set([...rejectedAi, ...memoryRejectedQuestionTexts(latestMemory), ...generatedQuestions.map((question) => question.question_text)]); const cleanExcluded = uniqueCategories([...rejectedCategories, ...recentSessionCategories, ...selectedSessionCategories, ...excludedCategories]); const approvedPool = approvedCategories.filter((category) => !cleanExcluded.some((excluded) => categoryKey(excluded) === categoryKey(category))); const fullTheme = [theme, `Build this as part of a full trivia session for ${sessionName || "this show"}.`, `Current target round: ${round.name}.`, round.description ? `Round direction: ${round.description}` : "", "Generate fresh, reusable categories for this round. Do not recycle categories from recent sessions or categories already selected for this build. If the approved category pool gets too narrow, create adjacent US-friendly categories with the same host vibe."].filter(Boolean).join("\n"); return axios.post("/api/generate-session-candidates", { sessionId: `full-${round.id}`, questionType: type, count, difficulty, theme: fullTheme, excludeUsed: true, avoidDuplicates: true, includeImagePrompt: false, excludeCategories: cleanExcluded, approvedCategories: approvedPool, rejectedCategories, rejectedQuestions: [...rejectedQuestions], sessionContext: buildAiSessionContext() }, { timeout: 120000 }); };
  ...
  const handleBuildFullSession = async () => {
    ...
    try {
      ...
      for (const round of sourceRounds) {
        ...
        const plan = fullSessionTypePlan(needed, template?.questionType || "mixed");
        for (const [type, typeCount] of Object.entries(plan)) {
          if (!typeCount) continue;
          const { data } = await requestGeneratedForFullRound({ round, type, count: typeCount, excludedCategories: generatedCategories, generatedQuestions });
          const generated = normalizeGeneratedForRound(data?.candidates, round, type).slice(0, typeCount);
      ...
      toast.success(`Built ${generatedQuestions.length} question${generatedQuestions.length === 1 ? "" : "s"} across the session`);
    } catch (error) {
