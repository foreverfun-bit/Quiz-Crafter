...
const BuildSession = () => {
  ...
  const handleImportFromOpenTrivia = async () => {
    ...
    setAiCandidates([]);
    try {
      const { data } = await axios.post("/api/generate-session-candidates", { mode: "opentdb_import", questionType: generateType, count: requestedTotal, difficulty, theme }, { timeout: 15000 });
      if (data?.source_notice) toast.info(data.source_notice);
      const imported = uniqueGeneratedQuestions(normalizeGeneratedCandidates(data?.candidates, activeRound.id, generateType), []).slice(0, requestedTotal);
      if (!imported.length) throw new Error("No questions came back. Try a different type or difficulty.");
