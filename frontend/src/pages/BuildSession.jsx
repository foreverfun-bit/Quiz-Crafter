...
const BuildSession = () => {
  ...
  const handleConvertGeneratedType = async (question, targetType) => { const currentType = normalizeType(question); if (!targetType || targetType === currentType) return; setActionQuestionId(question.id); try { const convertTheme = [theme, `Rewrite this exact ${questionTypeLabel(currentType)} question as a ${questionTypeLabel(targetType)} question.`, "This is a format conversion only. Do not replace it with a different trivia fact, different subject, different category, or different fun fact.", targetType === "true_false" ? "For true/false, turn the same fact into one clean verifiable claim and set correct_answer to exactly True or False." : targetType === "multiple_choice" ? "For multiple choice, keep the original correct answer as the correct answer and add three plausible wrong answers." : "For written answer, keep the original correct answer and make the clue fair without answer options.", `Original category: ${question.category || "General"}`, `Original question: ${question.question_text}`, `Original answer: ${question.correct_answer}`, question.fun_fact ? `Original fun fact: ${question.fun_fact}` : "", "Return only a rewritten version of this same question in the new format."].filter(Boolean).join("\n"); const { data } = await requestGeneratedQuestions({ replacement: question, rejectCurrent: false, typeOverride: targetType, themeOverride: convertTheme, allowCurrentCategory: true, lockedCategoriesOverride: [question.category].filter(Boolean), avoidDuplicatesOverride: false }); const converted = normalizeGeneratedCandidates(data?.candidates, activeRound.id, targetType).map((candidate) => normalizeQuestion({ ...candidate, id: question.id, category: question.category || candidate.category, fun_fact: candidate.fun_fact || question.fun_fact, isGenerated: true }, targetType))[0]; if (!converted) throw new Error("No converted question came back."); setQuestions((prev) => prev.map((item) => String(item.id) === String(question.id) ? converted : item)); toast.success(`Converted to ${questionTypeLabel(targetType)}`); } catch (error) { console.error("Convert generated question type error:", error); toast.error(error.response?.data?.error || error.message || "Failed to convert question type"); } finally { setActionQuestionId(null); } };
