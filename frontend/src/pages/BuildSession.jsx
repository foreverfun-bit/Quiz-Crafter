...
import { uploadQuestionAudio, uploadQuestionMedia } from "../lib/mediaUpload";
...
// Open Trivia DB's own category list (opentdb.com/api_category.php) --
// stable and rarely changes, so it's hardcoded rather than fetched live.
...
const buildStateFromSavedSession = (session) => {
  ...
  const questions = entries.map((entry) => {
    ...
    return normalizeQuestion({ ...entry.question, id, question_type: entry.type === "picture" ? "written" : entry.type, image_url: entry.question?.image_url || entry.question?.media_url || "", image_timing: entry.question?.image_timing || entry.question?.image_display_timing, isGenerated: Boolean(entry.question?.is_generated) }, entry.type);
  });
...
const BuildSession = ({ sessionIdProp, onGoLive } = {}) => {
  ...
  useEffect(() => {
    ...
    if (cachedCurrentProject !== undefined) { redirectTo(cachedCurrentProject); return; }
  ...
  const [buildAvoidedCategories, setBuildAvoidedCategories] = useState([]);
  ...
  const [otdbType, setOtdbType] = useState("multiple_choice");
  const [otdbDifficulty, setOtdbDifficulty] = useState("");
  const [otdbCount, setOtdbCount] = useState("10");
  ...
  const handleBrowseLibrary = () => { setUsedQuestionIds(readUsedIds()); setUnusedQuestionIds(readUnusedIds()); setQuestionMemory(readQuestionMemory()); setShowLibrary(true); closeWriteForm(); setShowAiPanel(false); };
  ...
  const draftCandidateCategories = useMemo(() => uniqueCategories(aiCandidates.map((candidate) => candidate.category)), [aiCandidates]);
  // Soft category exclusions should also cover categories the AI already drafted but the host
  ...
  // Rounds silently default to "mixed" (see normalizeRounds) with nowhere in
  // this page to see or change that -- a host who never touches this had no
  // way to know "mixed" was cycling multiple choice/true-false/written into
  // what they assumed was a single-type round.
  const handleSetRoundQuestionType = (roundId, questionType) => setRounds((prev) => prev.map((round) => (round.id === roundId ? { ...round, questionType } : round)));
  const handleMoveRound = (roundId, direction) => setRounds((prev) => { const list = [...prev]; const index = list.findIndex((round) => round.id === roundId); const nextIndex = index + direction; if (index < 0 || nextIndex < 0 || nextIndex >= list.length) return prev; [list[index], list[nextIndex]] = [list[nextIndex], list[index]]; return list; });
  ...
  const duplicateQuestion = (roundId, questionId) => {
    const source = questions.find((question) => String(question.id) === String(questionId));
    if (!source) return;
    const duplicate = { ...source, id: `dup-${Date.now()}-${Math.random().toString(36).slice(2, 8)}` };
    setQuestions((prev) => [...prev, duplicate]);
    setRounds((prev) => prev.map((round) => {
      if (round.id !== roundId) return round;
      const ids = round.questionIds || [];
      const index = ids.findIndex((id) => String(id) === String(questionId));
      const nextIds = [...ids];
      nextIds.splice(index === -1 ? nextIds.length : index + 1, 0, duplicate.id);
      return { ...round, questionIds: nextIds };
    }));
  ...
  const buildApprovedCategoryScope = ({ softExclusions = [], lockedCategories = [] } = {}) => {
    ...
    const hardExcluded = uniqueCategories([...rejectedCategories, ...buildAvoidedCategories]).filter((category) => !lockedKeys.has(categoryKey(category)));
  ...
  const handleAssistWriteQuestion = async () => {
    const type = normalizeType(writeForm);
    const hasQuestion = Boolean(normalizeText(writeForm.question_text));
    if (hasQuestion) {
      const missing = [];
      if (!normalizeText(writeForm.correct_answer)) missing.push("the correct answer");
      if (type === "multiple_choice" && cleanList(writeForm.incorrect_answers).length < 2) missing.push("three plausible, comparable wrong answers");
      if (!normalizeText(writeForm.fun_fact)) missing.push("a short fun fact");
      ...
      try {
        ...
        const { data } = await axios.post("/api/host-assistant", {
          ...
          context: {
            ...
            approvedCategories, rejectedCategories, recentSessionCategories,
            questions: [{ category: writeForm.category, question_text: writeForm.question_text, correct_answer: writeForm.correct_answer, question_type: type, incorrect_answers: cleanList(writeForm.incorrect_answers).join("; "), fun_fact: writeForm.fun_fact }],
            hostStyleProfile: latestStyle,
          },
        }, { timeout: 90000 });
        const candidate = data?.candidate;
        if (!candidate) throw new Error("No AI draft came back.");
        const wrongAnswers = candidate.options?.length ? candidate.options : parseAnswerOptions(candidate.incorrect_answers);
        setWriteForm((prev) => ({
          ...prev,
          correct_answer: normalizeText(prev.correct_answer) || candidate.correct_answer || prev.correct_answer,
          incorrect_answers: type === "multiple_choice" && cleanList(prev.incorrect_answers).length < 2 ? [...wrongAnswers, "", "", ""].slice(0, 3) : prev.incorrect_answers,
          fun_fact: normalizeText(prev.fun_fact) || candidate.fun_fact || prev.fun_fact,
        }));
        toast.success("Filled in the missing pieces");
      } catch (error) {
        console.error("Free write AI complete error:", error);
        toast.error(error.response?.data?.error || error.message || "Failed to complete question");
      } finally {
    ...
    setDraftGenerating(true);
    try {
      const assistTheme = [theme, writeForm.category ? `Use category: ${writeForm.category}` : ""].filter(Boolean).join("\n");
      const writeCategory = normalizeText(writeForm.category);
      const draftCategoryExclusions = writeCategory ? categoriesInUseThisBuild.filter((category) => categoryKey(category) !== categoryKey(writeCategory)) : categoriesInUseThisBuild;
      const { excludedCategories: draftExcludedCategories, approvedPool: draftApprovedPool } = buildApprovedCategoryScope({ softExclusions: draftCategoryExclusions, lockedCategories: writeCategory ? [writeCategory] : [] });
      const { data } = await axios.post("/api/generate-session-candidates", { sessionId: `write-${activeRound.id}`, questionType: type, count: 1, difficulty, theme: assistTheme, excludeUsed: true, avoidDuplicates: true, includeImagePrompt: false, excludeCategories: draftExcludedCategories, approvedCategories: draftApprovedPool, rejectedCategories, lockedCategories: writeCategory ? [writeCategory] : [], rejectedQuestions: [...rejectedAi], venueId: selectedVenueId || null }, { timeout: 90000 });
      const candidate = normalizeGeneratedCandidates(data?.candidates, activeRound.id, type)[0];
      if (!candidate) throw new Error("No AI draft came back. Try adding a category or theme.");
      const wrongAnswers = candidate.options?.length ? candidate.options : parseAnswerOptions(candidate.incorrect_answers);
      setWriteForm((prev) => ({ ...prev, question_type: type, category: candidate.category || prev.category, question_text: candidate.question_text || prev.question_text, correct_answer: candidate.correct_answer || prev.correct_answer, incorrect_answers: type === "multiple_choice" ? [...wrongAnswers, "", "", ""].slice(0, 3) : prev.incorrect_answers, fun_fact: candidate.fun_fact || prev.fun_fact, image_url: candidate.image_url || prev.image_url, image_timing: prev.image_timing || "initial" }));
      toast.success("AI draft added");
    } catch (error) {
      console.error("Free write AI assist error:", error);
      toast.error(error.response?.data?.error || error.message || "Failed to draft question");
    } finally {
      setDraftGenerating(false);
    }
  };
  const handleAskCoHost = async () => { const request = normalizeText(cohostPrompt); if (!request) return toast.error("Ask the co-host what you need help with"); const nextMessages = [...cohostMessages, { role: "user", content: request }].slice(-10); setCohostMessages(nextMessages); setCohostPrompt(""); setCohostLoading(true); try { const { excludedCategories, approvedPool } = buildApprovedCategoryScope({ softExclusions: categoriesInUseThisBuild }); const latestStyle = readHostStyleProfile(); setHostStyleProfile(latestStyle); const { data } = await axios.post("/api/host-assistant", { mode: "build_cohost", request, context: { session: { name: sessionName, session_date: sessionDate, difficulty, activeRound: activeRound.name, targetType: activeRound?.questionType && activeRound.questionType !== "mixed" ? activeRound.questionType : generateType }, venue: selectedVenueId ? { name: selectedVenue?.name || selectedVenue?.nightName || "", insights: venueInsights } : null, build: buildAiSessionContext(), approvedCategories: approvedPool.length ? approvedPool : approvedCategories, rejectedCategories: uniqueCategories([...rejectedCategories, ...excludedCategories]), recentSessionCategories, conversation: nextMessages, questions: activeRoundQuestions.map((question) => ({ category: question.category, question_text: question.question_text, correct_answer: question.correct_answer, question_type: normalizeType(question) })), hostStyleProfile: latestStyle } }, { timeout: 90000 }); const reply = data?.answer || "I drafted a few suggestions for this round."; setCohostAnswer(reply); setCohostMessages((prev) => [...prev, { role: "assistant", content: reply }].slice(-12)); setCohostCandidates(normalizeCoHostCandidates(data?.candidates)); toast.success("Co-host response ready"); } catch (error) { console.error("AI co-host error:", error); setCohostMessages((prev) => [...prev, { role: "assistant", content: "I could not finish that request. Try asking again with a tighter direction or fewer requested questions." }].slice(-12)); toast.error(error.response?.data?.error || error.message || "Failed to ask the co-host"); } finally { setCohostLoading(false); } };
  ...
  // Pulls questions straight from Open Trivia DB (a community-vetted bank),
  // skipping the AI rewrite step entirely -- unlike handleGenerateForRound,
  // nothing here is written or verified by a model, so there's no chance of
  // it inventing a wrong answer. Still lands in the same candidate review
  // list so the host gives everything one look before it's added.
  const handleImportFromOpenTrivia = async () => {
    const requestedTotal = Math.max(1, Math.min(50, Number(otdbCount) || 10));
    setGenerating(true);
    setGenerationStatus(`Importing from Open Trivia DB...`);
    setAiCandidates([]);
    try {
      const { data } = await axios.post("/api/generate-session-candidates", { mode: "opentdb_import", questionType: otdbType, count: requestedTotal, difficulty: otdbDifficulty, categoryId: otdbCategory }, { timeout: 15000 });
      if (data?.source_notice) toast.info(data.source_notice);
      const imported = uniqueGeneratedQuestions(normalizeGeneratedCandidates(data?.candidates, activeRound.id, otdbType), []).slice(0, requestedTotal);
      if (!imported.length) throw new Error("No questions came back. Try a different category, type, or difficulty.");
      setAiCandidates(imported);
      toast.success(`Imported ${imported.length} question${imported.length === 1 ? "" : "s"} from Open Trivia DB`);
    } catch (error) {
      console.error("Open Trivia DB import error:", error);
      toast.error(error.response?.data?.error || error.message || "Failed to import from Open Trivia DB");
    } finally {
      setGenerating(false);
      setGenerationStatus("");
    }
  ...
  const handleAiStyleFeedback = async (candidate, action) => {
    const labels = { keep: "Kept as your style", too_easy: "Marked too easy -- finding a harder replacement...", too_hard: "Marked too hard -- finding an easier replacement...", too_common: "Marked too common -- finding a fresher replacement...", not_my_style: "Marked not your style -- finding a replacement...", more_like_this: "Style memory updated", used_before: "Marked as already used -- finding something fresh...", ambiguous: "Marked as ambiguous -- finding a clear-cut replacement..." };
    ...
    try {
      ...
      if (["too_common", "not_my_style", "too_hard", "used_before", "ambiguous"].includes(action)) {
        const status = action === "too_common" ? "too_common" : action === "used_before" ? "used_recently" : "needs_rewrite";
        const nextMemory = upsertQuestionMemory(candidate, { status, note: labels[action] }, readQuestionMemory());
    ...
    else if (action === "too_common" || action === "not_my_style" || action === "used_before" || action === "ambiguous") handleRegenerateAiCandidate(candidate, "question");
  };
  ...
  // Unlike handleRegenerateAiCandidate's "category" scope, this never
  // touches the question text or answer -- it only asks the AI to pick a
  // better-fitting category label. That matters most for Open Trivia DB
  // imports, whose own category tagging is often just "General Knowledge,"
  // and where rewriting the question would defeat the point of importing
  // already-correct content in the first place.
  const handleRecategorizeAiCandidate = async (candidate) => {
    setAiActionId(candidate.id);
    try {
      const { data } = await axios.post("/api/generate-session-candidates", { mode: "categorize", questionText: candidate.question_text, correctAnswer: candidate.correct_answer, currentCategory: candidate.category, approvedCategories }, { timeout: 15000 });
      const nextCategory = normalizeText(data?.category);
      if (!nextCategory) throw new Error("No category came back");
      setAiCandidates((prev) => prev.map((item) => String(item.id) === String(candidate.id) ? { ...item, category: canonicalCategory(nextCategory, approvedCategories) } : item));
      toast.success(`Recategorized as ${nextCategory}`);
    } catch (error) {
      console.error("Recategorize AI candidate error:", error);
      toast.error(error.response?.data?.error || error.message || "Failed to recategorize");
    } finally {
      setAiActionId(null);
    }
  };
  // Runs the same per-question recategorize call across every current
  // candidate at once -- mainly for Open Trivia DB imports, where a whole
  // batch can land tagged "General Knowledge" and fixing them one at a time
  // isn't realistic for a 20-50 question import.
  const handleRecategorizeAllAiCandidates = async () => {
    if (!aiCandidates.length) return;
    setRecategorizingAll(true);
    try {
      const targets = aiCandidates;
      const results = await Promise.all(targets.map((candidate) => axios.post("/api/generate-session-candidates", { mode: "categorize", questionText: candidate.question_text, correctAnswer: candidate.correct_answer, currentCategory: candidate.category, approvedCategories }, { timeout: 15000 }).then((response) => ({ id: candidate.id, category: normalizeText(response.data?.category) })).catch((error) => { console.warn("Bulk recategorize error:", error.message || error); return null; })));
      const byId = new Map(results.filter(Boolean).filter((item) => item.category).map((item) => [String(item.id), canonicalCategory(item.category, approvedCategories)]));
      if (!byId.size) throw new Error("No categories came back");
      setAiCandidates((prev) => prev.map((item) => byId.has(String(item.id)) ? { ...item, category: byId.get(String(item.id)) } : item));
      toast.success(`Recategorized ${byId.size} of ${targets.length} question${targets.length === 1 ? "" : "s"}`);
    } catch (error) {
  ...
  return <div className="p-4 lg:p-6 max-w-7xl mx-auto animate-fade-in" data-testid="build-session-page">
    ...
    <BuildAssistantStrip onAsk={askFloatingCopilot} onBuildRound={handleBuildActiveRound} onImportOpenTrivia={handleImportFromOpenTrivia} otdbCategory={otdbCategory} setOtdbCategory={setOtdbCategory} otdbType={otdbType} setOtdbType={setOtdbType} otdbDifficulty={otdbDifficulty} setOtdbDifficulty={setOtdbDifficulty} otdbCount={otdbCount} setOtdbCount={setOtdbCount} activeRoundName={activeRound.name} undoSnapshot={builderUndoSnapshot} onUndo={restoreBuilderUndo} loading={builderChatLoading || generating} generationStatus={generationStatus} generationMode={generationMode} setGenerationMode={setGenerationMode} />
    <BuildAssistantResults preview={chatPreview} onAcceptPreview={acceptChatPreview} onDiscardPreview={() => setChatPreview(null)} aiCandidates={aiCandidates} aiActionId={aiActionId} mediaQuestionId={mediaQuestionId} settingsQuestionId={settingsQuestionId} onAddToSession={handleAddAiCandidateToSession} onAddAll={handleAddAllAiCandidates} onSaveToLibrary={handleSaveAiCandidateToLibrary} onRegenerate={handleRegenerateAiCandidate} onRecategorize={handleRecategorizeAiCandidate} onRecategorizeAll={handleRecategorizeAllAiCandidates} recategorizingAll={recategorizingAll} onConvertType={handleConvertAiCandidateType} onTuneDifficulty={handleTuneAiCandidateDifficulty} onStyleFeedback={handleAiStyleFeedback} onOpenMedia={setMediaQuestionId} onOpenSettings={setSettingsQuestionId} onEditCandidate={updateAiCandidate} />
    <div className="space-y-5"><RoundDropdown rounds={rounds} activeRoundId={activeRoundId} menuOpen={roundMenuOpen} setMenuOpen={setRoundMenuOpen} onSelect={setActiveRoundId} onRename={handleRenameRound} onDescribe={handleDescribeRound} onSetQuestionType={handleSetRoundQuestionType} onDelete={handleDeleteRound} onAdd={handleAddRound} onMoveRound={handleMoveRound} />
      <Card className="glass-card overflow-hidden border-[#71E0DC]/15"><CardHeader className="pb-3 border-b border-white/10"><div><CardTitle className="text-white flex items-center gap-2"><Layers size={20} className="text-[#71E0DC]" />{activeRound.name}</CardTitle><CardDescription className="text-zinc-500">{activeRoundQuestions.length} question{activeRoundQuestions.length === 1 ? "" : "s"} selected. Ask Quiz Crafter above to edit this workspace.</CardDescription></div></CardHeader><CardContent className="p-4 space-y-4"><BuilderToolbar showWriteForm={showWriteForm} onToggleWrite={() => (showWriteForm ? closeWriteForm() : openWriteForm())} showLibrary={showLibrary} onBrowseLibrary={handleBrowseLibrary} />{showWriteForm && <FreeWriteModal form={writeForm} setForm={setWriteForm} categories={approvedCategories} onSubmit={handleAddCustomQuestion} onCancel={closeWriteForm} onAssist={handleAssistWriteQuestion} draftGenerating={draftGenerating} />}<SelectedRoundList questions={activeRoundQuestions} rounds={safeRounds} activeRoundId={activeRound.id} mediaQuestionId={mediaQuestionId} settingsQuestionId={settingsQuestionId} actionQuestionId={actionQuestionId} onOpenMedia={setMediaQuestionId} onOpenSettings={setSettingsQuestionId} onCloseMedia={() => setMediaQuestionId(null)} onMoveWithinRound={moveQuestionWithinRound} onMoveToRound={moveQuestionToRound} onRemove={(questionId) => removeFromRound(activeRound.id, questionId)} onDuplicate={(questionId) => duplicateQuestion(activeRound.id, questionId)} onUpdate={updateQuestion} onSaveToLibrary={handleSaveQuestionToLibrary} onRefresh={handleRefreshGenerated} onConvertType={handleConvertGeneratedType} onTuneDifficulty={handleTuneGeneratedDifficulty} onChatEdit={handleChatEditQuestion} onRejectQuestion={handleRejectQuestion} onRejectCategory={handleRejectCategory} /></CardContent></Card>
    </div>
    ...
    {mediaQuestion && <MediaModal question={mediaQuestion} onClose={() => setMediaQuestionId(null)} onSave={(patch) => { if (isCandidateId(mediaQuestion.id)) updateAiCandidate(mediaQuestion.id, patch); else updateQuestion(mediaQuestion.id, patch); setMediaQuestionId(null); toast.success(patch.image_url || patch.audio_url ? "Media saved" : "Media removed"); }} />}
...
const VenueInsightsPanel = ({ venueName, insights, loading }) => {
  ...
  return <Card className="glass-card mb-4 border-[#71E0DC]/15"><CardContent className="p-4 space-y-3">
    <button type="button" onClick={() => setCollapsed((value) => !value)} className="flex w-full items-center justify-between gap-2 text-left">
      <span className="flex items-center gap-2 text-white font-semibold"><MapPin size={16} className="text-[#71E0DC]" />What {venueName} has taught the AI</span>
      <ChevronDown size={18} className={`flex-shrink-0 text-zinc-400 transition-transform ${collapsed ? "-rotate-90" : ""}`} />
    </button>
    {!collapsed && <>
...
const roundTypeShortLabel = (value) => (value && value !== "mixed" ? questionTypes.find((type) => type.value === value)?.shortLabel || "Question" : "Mixed");
const RoundDropdown = ({ rounds, activeRoundId, menuOpen, setMenuOpen, onSelect, onRename, onDescribe, onSetQuestionType, onDelete, onAdd, onMoveRound }) => {
  const [manageOpen, setManageOpen] = useState(false);
  const activeRound = rounds.find((round) => round.id === activeRoundId) || rounds[0];
  return <div className="relative z-20 flex items-center gap-2"><Button type="button" onClick={() => setMenuOpen((value) => !value)} className="h-10 border border-white/10 bg-zinc-950/70 px-3 text-zinc-100 shadow-lg shadow-black/20 hover:border-[#71E0DC]/40 hover:bg-zinc-900"><span className="font-semibold truncate max-w-[190px]">{activeRound?.name || "Select Round"}</span><Badge className="ml-2 bg-[#71E0DC]/15 text-[#71E0DC] border border-[#71E0DC]/20">{activeRound?.questionIds?.length || 0}</Badge><ChevronDown size={16} className={`ml-2 transition-transform ${menuOpen ? "rotate-180" : ""}`} /></Button>{menuOpen && <div className="absolute left-0 top-12 w-[300px] rounded-xl border border-white/10 bg-zinc-950/95 shadow-2xl shadow-black/50 p-2"><div className="flex items-center justify-between gap-2 px-3 py-2"><p className="text-xs text-zinc-500">Rounds</p><button type="button" onClick={() => setManageOpen((value) => !value)} className={`text-xs font-semibold ${manageOpen ? "text-[#71E0DC]" : "text-zinc-400 hover:text-white"}`}>{manageOpen ? "Done" : "Manage"}</button></div><div className="space-y-1">{rounds.map((round, index) => { const active = round.id === activeRoundId; return <div key={round.id} className={`rounded-lg border transition-colors ${active ? "border-[#71E0DC]/30 bg-[#71E0DC]/10 text-white" : "border-transparent text-zinc-300 hover:bg-zinc-900/70 hover:text-white"}`}><div className="flex items-center gap-1 px-1 py-1"><button type="button" onClick={() => onSelect(round.id)} className="min-w-0 flex-1 px-2 py-1 text-left"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold">{round.name}</p><p className="text-xs text-zinc-500">Round {index + 1} &middot; {roundTypeShortLabel(round.questionType)}</p></div><Badge className="bg-zinc-800 text-zinc-300">{round.questionIds?.length || 0}</Badge></div></button>{onMoveRound && <div className="flex flex-col flex-shrink-0"><button type="button" title="Move round up" aria-label="Move round up" onClick={() => onMoveRound(round.id, -1)} disabled={index === 0} className="p-0.5 text-zinc-500 hover:text-white disabled:opacity-30 disabled:hover:text-zinc-500"><ArrowUp size={14} /></button><button type="button" title="Move round down" aria-label="Move round down" onClick={() => onMoveRound(round.id, 1)} disabled={index === rounds.length - 1} className="p-0.5 text-zinc-500 hover:text-white disabled:opacity-30 disabled:hover:text-zinc-500"><ArrowDown size={14} /></button></div>}</div>{manageOpen && active && <div className="mx-2 mb-2 rounded-lg border border-[#71E0DC]/20 bg-zinc-950/70 p-2 space-y-2"><Label className="text-xs text-zinc-500">Editing this round</Label><div className="flex items-center gap-2"><Input value={round.name} onChange={(e) => onRename(round.id, e.target.value)} className="h-9 bg-zinc-950/70 border-white/10 text-white text-sm" /><Button title="Delete round" aria-label="Delete round" size="sm" variant="outline" onClick={() => onDelete(round.id)} className="h-9 w-9 p-0 border-zinc-700 text-zinc-400 hover:text-red-300"><Trash2 size={14} /></Button></div><Textarea value={round.description || ""} onChange={(e) => onDescribe(round.id, e.target.value)} placeholder="Round description or host notes" className="min-h-[64px] bg-zinc-950/70 border-white/10 text-white text-sm" />{onSetQuestionType && <label className="block text-xs text-zinc-500">Question type<select value={round.questionType || "mixed"} onChange={(e) => onSetQuestionType(round.id, e.target.value)} className="mt-1 h-9 w-full rounded-md bg-zinc-950/70 border border-white/10 px-2 text-white text-sm outline-none focus:border-[#71E0DC]/60"><option value="mixed">Mixed</option><option value="written">Written</option><option value="multiple_choice">Multiple Choice</option><option value="true_false">True/False</option></select></label>}</div>}</div>; })}</div>{manageOpen && <><div className="my-2 h-px bg-white/10" /><Button type="button" onClick={onAdd} className="w-full h-9 gradient-btn"><Plus size={15} className="mr-2" />Add Round</Button><p className="px-2 pt-2 text-[11px] text-zinc-500">Select any round above to edit it in place. Use the arrows to reorder rounds.</p></>}</div>}</div>;
};
const BuilderToolbar = ({ showWriteForm, onToggleWrite, showLibrary, onBrowseLibrary }) => <div className="rounded-xl border border-white/10 bg-zinc-950/35 p-3"><div className="flex flex-col md:flex-row md:items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-zinc-500">Manual tools</p><p className="text-sm text-zinc-500 mt-1">Use these when you want to bypass the assistant.</p></div><div className="flex flex-wrap gap-2"><AddModeButton active={showWriteForm} icon={Pencil} label="Write" onClick={onToggleWrite} /><AddModeButton active={showLibrary} icon={List} label="Library" onClick={onBrowseLibrary} /></div></div></div>;
...
const AiCandidateCard = ({ candidate, busy, mediaActive, settingsActive, onAddToSession, onSaveToLibrary, onRegenQuestion, onRegenCategory, onRecategorize, onConvertType, onTuneDifficulty, onStyleFeedback, onOpenMedia, onOpenSettings, onEditCandidate }) => {
...
const MediaModal = ({ question, onClose, onSave }) => {
  ...
  const handleAudioFile = async (file) => {
    if (!file || !file.type.startsWith("audio/")) return toast.error("Choose an audio file");
    setUploadingAudio(true);
    try {
      setAudioUrl(await uploadQuestionAudio(file));
    } catch (error) {
      console.error("Upload audio error:", error);
      toast.error(error.message || "Failed to upload audio clip");
    } finally {
