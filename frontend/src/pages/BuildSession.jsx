...
const BuildSession = () => {
  ...
  const requestGeneratedQuestions = async ({ replacement = null, rejectCurrent = false, difficultyOverride = null, themeOverride = null, allowCurrentCategory = false, lockedCategoriesOverride = [], typeOverride = null } = {}) => { const count = replacement ? 1 : Math.max(1, Math.min(20, Number(generateCount) || 3)); const type = typeOverride || (replacement ? normalizeType(replacement) : generateType); const latestMemory = readQuestionMemory(); setQuestionMemory(latestMemory); const rejectedQuestions = new Set([...rejectedAi, ...memoryRejectedQuestionTexts(latestMemory)]); if (rejectCurrent && replacement?.question_text) rejectedQuestions.add(replacement.question_text); const lockedCategoryKeys = new Set(uniqueCategories(lockedCategoriesOverride).map(categoryKey)); const categoryExclusions = allowCurrentCategory && replacement ? selectedSessionCategories.filter((category) => categoryKey(category) !== categoryKey(replacement.category)) : selectedSessionCategories; const excludedCategories = uniqueCategories([...rejectedCategories, ...categoryExclusions].filter((category) => !lockedCategoryKeys.has(categoryKey(category)))); const approvedPool = approvedCategories.filter((category) => !excludedCategories.some((excluded) => categoryKey(excluded) === categoryKey(category))); return axios.post("/api/generate-session-candidates", { sessionId: `build-${activeRound.id}`, questionType: type, count, difficulty: difficultyOverride || difficulty, theme: themeOverride ?? theme, excludeUsed: true, avoidDuplicates: true, includeImagePrompt: false, excludeCategories: excludedCategories, approvedCategories: approvedPool, rejectedCategories, lockedCategories: uniqueCategories(lockedCategoriesOverride), rejectedQuestions: [...rejectedQuestions], sessionContext: buildAiSessionContext() }, { timeout: 90000 }); };
  ...
  const handleConvertAiCandidateType = async (candidate, targetType) => { const currentType = normalizeType(candidate); if (!targetType || targetType === currentType) return; setAiActionId(candidate.id); try { const convertTheme = [theme, `Convert this ${questionTypeLabel(currentType)} candidate into a ${questionTypeLabel(targetType)} question.`, "Keep the same category, core trivia idea, answer knowledge, and fun fact value, but rewrite the question so it naturally fits the new format.", targetType === "true_false" ? "For true/false, write a clean verifiable claim and set correct_answer to exactly True or False." : targetType === "multiple_choice" ? "For multiple choice, include one correct answer and three plausible wrong answers." : "For written answer, make the clue fair without answer options and avoid exact-spelling traps.", `Original category: ${candidate.category || "General"}`, `Original question: ${candidate.question_text}`, `Original answer: ${candidate.correct_answer}`, candidate.fun_fact ? `Original fun fact: ${candidate.fun_fact}` : ""].filter(Boolean).join("\n"); const { data } = await requestGeneratedQuestions({ replacement: candidate, rejectCurrent: false, typeOverride: targetType, themeOverride: convertTheme, allowCurrentCategory: true, lockedCategoriesOverride: [candidate.category].filter(Boolean) }); const converted = normalizeGeneratedCandidates(data?.candidates, activeRound.id, targetType).map((item) => normalizeQuestion({ ...item, id: candidate.id, category: candidate.category || item.category, isGenerated: true }, targetType))[0]; if (!converted) throw new Error("No converted question came back."); setAiCandidates((prev) => prev.map((item) => String(item.id) === String(candidate.id) ? converted : item)); toast.success(`Converted to ${questionTypeLabel(targetType)}`); } catch (error) { console.error("Convert AI candidate type error:", error); toast.error(error.response?.data?.error || error.message || "Failed to convert question type"); } finally { setAiActionId(null); } };
  ...
  const handleConvertGeneratedType = async (question, targetType) => { const currentType = normalizeType(question); if (!targetType || targetType === currentType) return; setActionQuestionId(question.id); try { const convertTheme = [theme, `Convert this ${questionTypeLabel(currentType)} question into a ${questionTypeLabel(targetType)} question.`, "Keep the same category and core trivia idea, but rewrite it so the new format feels intentional for live bar trivia.", targetType === "true_false" ? "For true/false, write a single clean claim and set correct_answer to exactly True or False." : targetType === "multiple_choice" ? "For multiple choice, include one correct answer and three plausible wrong answers." : "For written answer, make the clue fair without answer options and avoid exact-spelling traps.", `Original category: ${question.category || "General"}`, `Original question: ${question.question_text}`, `Original answer: ${question.correct_answer}`, question.fun_fact ? `Original fun fact: ${question.fun_fact}` : ""].filter(Boolean).join("\n"); const { data } = await requestGeneratedQuestions({ replacement: question, rejectCurrent: false, typeOverride: targetType, themeOverride: convertTheme, allowCurrentCategory: true, lockedCategoriesOverride: [question.category].filter(Boolean) }); const generated = normalizeGeneratedCandidates(data?.candidates, activeRound.id, targetType).map((candidate) => normalizeQuestion({ ...candidate, category: question.category || candidate.category, isGenerated: true }, targetType)); if (!generated.length) throw new Error("No converted question came back."); addGeneratedToRound(generated, question); toast.success(`Converted to ${questionTypeLabel(targetType)}`); } catch (error) { console.error("Convert generated question type error:", error); toast.error(error.response?.data?.error || error.message || "Failed to convert question type"); } finally { setActionQuestionId(null); } };
  ...
  return <div className="p-4 lg:p-6 max-w-7xl mx-auto animate-fade-in" data-testid="build-session-page">
    ...
    <div className="space-y-5"><RoundDropdown rounds={rounds} activeRoundId={activeRoundId} menuOpen={roundMenuOpen} setMenuOpen={setRoundMenuOpen} onSelect={setActiveRoundId} onRename={handleRenameRound} onDescribe={handleDescribeRound} onDelete={handleDeleteRound} onAdd={handleAddRound} />
      <Card className="glass-card overflow-hidden"><CardHeader className="pb-3 border-b border-white/10"><div><CardTitle className="text-white flex items-center gap-2"><Layers size={20} className="text-[#71E0DC]" />{activeRound.name}</CardTitle><CardDescription className="text-zinc-500">{activeRoundQuestions.length} question{activeRoundQuestions.length === 1 ? "" : "s"} selected</CardDescription></div></CardHeader><CardContent className="p-4 space-y-4"><BuilderToolbar showAiPanel={showAiPanel} setShowAiPanel={setShowAiPanel} showWriteForm={showWriteForm} setShowWriteForm={setShowWriteForm} showLibrary={showLibrary} onBrowseLibrary={handleBrowseLibrary} />{showAiPanel && <AiGenerateModal theme={theme} setTheme={setTheme} difficulty={difficulty} setDifficulty={setDifficulty} generateType={generateType} setGenerateType={setGenerateType} generateCount={generateCount} setGenerateCount={setGenerateCount} generating={generating} aiCandidates={aiCandidates} aiActionId={aiActionId} onGenerate={handleGenerateForRound} onAddToSession={handleAddAiCandidateToSession} onSaveToLibrary={handleSaveAiCandidateToLibrary} onRegenerate={handleRegenerateAiCandidate} onConvertType={handleConvertAiCandidateType} onClose={() => setShowAiPanel(false)} />}{showWriteForm && <FreeWriteModal form={writeForm} setForm={setWriteForm} categories={approvedCategories} onSubmit={handleAddCustomQuestion} onCancel={() => setShowWriteForm(false)} onAssist={handleAssistWriteQuestion} draftGenerating={draftGenerating} />}<SelectedRoundList questions={activeRoundQuestions} rounds={rounds} activeRoundId={activeRound.id} mediaQuestionId={mediaQuestionId} settingsQuestionId={settingsQuestionId} actionQuestionId={actionQuestionId} onOpenMedia={setMediaQuestionId} onOpenSettings={setSettingsQuestionId} onCloseMedia={() => setMediaQuestionId(null)} onMoveWithinRound={moveQuestionWithinRound} onMoveToRound={moveQuestionToRound} onRemove={(questionId) => removeFromRound(activeRound.id, questionId)} onUpdate={updateQuestion} onSaveToLibrary={handleSaveQuestionToLibrary} onRefresh={handleRefreshGenerated} onConvertType={handleConvertGeneratedType} onTuneDifficulty={handleTuneGeneratedDifficulty} onRejectQuestion={handleRejectQuestion} onRejectCategory={handleRejectCategory} /></CardContent></Card>
    </div>
...
const BuilderToolbar = ({ showAiPanel, setShowAiPanel, showWriteForm, setShowWriteForm, showLibrary, onBrowseLibrary }) => <div className="rounded-xl border border-white/10 bg-zinc-950/45 p-4"><div className="flex flex-col md:flex-row md:items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wide text-[#71E0DC]">Add questions</p><p className="text-sm text-zinc-500 mt-1">Choose how you want to build this round.</p></div><div className="flex flex-wrap gap-2"><AddModeButton active={showAiPanel} icon={Sparkles} label="AI" onClick={() => { setShowAiPanel((value) => !value); setShowWriteForm(false); }} /><AddModeButton active={showWriteForm} icon={Pencil} label="Write" onClick={() => { setShowWriteForm((value) => !value); setShowAiPanel(false); }} /><AddModeButton active={showLibrary} icon={List} label="Library" onClick={() => { onBrowseLibrary(); setShowAiPanel(false); setShowWriteForm(false); }} /></div></div></div>;
