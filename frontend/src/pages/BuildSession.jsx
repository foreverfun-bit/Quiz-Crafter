...
const BuildSession = () => {
  ...
  const collectFullBuildCandidates = async ({ round, type, needed, generatedCategories, generatedQuestions }) => { const accepted = []; for (let attempt = 1; accepted.length < needed && attempt <= 2; attempt += 1) { const missing = needed - accepted.length; const requestCount = Math.min(20, Math.max(missing + 4, Math.ceil(missing * 2))); const { data } = await requestGeneratedForFullRound({ round, type, count: requestCount, excludedCategories: [...generatedCategories, ...accepted.map((question) => question.category)], generatedQuestions: [...generatedQuestions, ...accepted], attempt }); const uniqueCandidates = uniqueGeneratedQuestions(normalizeGeneratedForRound(data?.candidates, round, type), [...generatedQuestions, ...accepted]); const preferredKeys = new Set(uniqueCandidates.filter(isPreferredFullBuildCandidate).map((question) => fingerprint(question.question_text))); const orderedCandidates = [...uniqueCandidates.filter((question) => preferredKeys.has(fingerprint(question.question_text))), ...uniqueCandidates.filter((question) => !preferredKeys.has(fingerprint(question.question_text)))].slice(0, missing); accepted.push(...orderedCandidates); } return accepted.slice(0, needed); };
