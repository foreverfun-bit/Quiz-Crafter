...
const BuildSession = () => {
  ...
  const requestGeneratedQuestions = async ({ replacement = null, rejectCurrent = false, difficultyOverride = null, themeOverride = null, allowCurrentCategory = false, lockedCategoriesOverride = [], typeOverride = null, countOverride = null, roundOverride = null, avoidDuplicatesOverride = true, generationModeOverride = null } = {}) => { const targetRound = roundOverride || activeRound; const mode = generationModeOverride || generationMode; const count = replacement ? Math.max(1, Math.min(12, Number(countOverride) || 1)) : Math.max(1, Math.min(20, Number(countOverride ?? generateCount) || 3)); const type = typeOverride || (replacement ? normalizeType(replacement) : generateType); const latestMemory = readQuestionMemory(); setQuestionMemory(latestMemory); const latestStyle = readHostStyleProfile(); setHostStyleProfile(latestStyle); const rejectedQuestions = new Set([...rejectedAi, ...memoryRejectedQuestionTexts(latestMemory)]); const rejectedAnswers = new Set(); if (rejectCurrent && replacement?.question_text) rejectedQuestions.add(replacement.question_text); if (rejectCurrent && replacement?.correct_answer) rejectedAnswers.add(replacement.correct_answer); const categoryExclusions = allowCurrentCategory && replacement ? selectedSessionCategories.filter((category) => categoryKey(category) !== categoryKey(replacement.category)) : selectedSessionCategories; const { excludedCategories, approvedPool } = buildApprovedCategoryScope({ softExclusions: categoryExclusions, lockedCategories: lockedCategoriesOverride }); return axios.post("/api/generate-session-candidates", { sessionId: `build-${targetRound.id}`, questionType: type, count, difficulty: difficultyOverride || difficulty, theme: themeOverride ?? theme, excludeUsed: true, avoidDuplicates: avoidDuplicatesOverride, includeImagePrompt: false, excludeCategories: excludedCategories, approvedCategories: approvedPool, rejectedCategories, lockedCategories: uniqueCategories(lockedCategoriesOverride), rejectedQuestions: [...rejectedQuestions], rejectedAnswers: [...rejectedAnswers], sessionContext: buildAiSessionContext(), generationMode: mode, hostStyleProfile: latestStyle }, { timeout: mode === "quick" ? 70000 : 90000 }); };
  ...
  const handleRefreshGenerated = async (question) => { setActionQuestionId(question.id); try { const { data } = await requestGeneratedQuestions({ replacement: question, rejectCurrent: true, allowCurrentCategory: true, countOverride: 6, themeOverride: [theme, "Refresh this question with a truly different trivia fact. Do not keep the same correct answer, subject, object, person, place, or topic angle.", `Rejected question: ${question.question_text}`, `Rejected answer: ${question.correct_answer}`].filter(Boolean).join("\n") }); const generated = normalizeGeneratedCandidates(data?.candidates, activeRound.id, normalizeType(question)).filter((candidate) => !isNearDuplicateQuestion(candidate, question)); if (!generated.length) throw new Error("No fresh replacement came back. Try changing the category or AI direction."); addGeneratedToRound([generated[0]], question); toast.success("Question refreshed"); } catch (error) { console.error("Refresh generated question error:", error); toast.error(error.response?.data?.error || error.message || "Failed to refresh question"); } finally { setActionQuestionId(null); } };
