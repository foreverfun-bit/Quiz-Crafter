...
const BuildSession = () => {
  ...
  const fullSessionTypePlan = (needed) => { const plan = { multiple_choice: 0, true_false: 0, written: 0 }; const order = ["multiple_choice", "true_false", "written", "multiple_choice", "true_false"]; for (let index = 0; index < needed; index += 1) plan[order[index % order.length]] += 1; return plan; };
  ...
  const handleBuildFullSession = async () => {
    ...
    try {
      ...
      for (const round of sourceRounds) {
        ...
        const plan = fullSessionTypePlan(needed);
