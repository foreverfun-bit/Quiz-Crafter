...
const BuildSession = () => {
  ...
  const requestGeneratedQuestions = async ({ replacement = null, rejectCurrent = false, difficultyOverride = null, themeOverride = null, allowCurrentCategory = false, lockedCategoriesOverride = [], typeOverride = null, avoidDuplicatesOverride = true } = {}) => { const count = replacement ? 1 : Math.max(1, Math.min(20, Number(generateCount) || 3)); const type = typeOverride || (replacement ? normalizeType(replacement) : generateType); const latestMemory = readQuestionMemory(); setQuestionMemory(latestMemory); const rejectedQuestions = new Set([...rejectedAi, ...memoryRejectedQuestionTexts(latestMemory)]); if (rejectCurrent && replacement?.question_text) rejectedQuestions.add(replacement.question_text); const lockedCategoryKeys = new Set(uniqueCategories(lockedCategoriesOverride).map(categoryKey)); const categoryExclusions = allowCurrentCategory && replacement ? selectedSessionCategories.filter((category) => categoryKey(category) !== categoryKey(replacement.category)) : selectedSessionCategories; const excludedCategories = uniqueCategories([...rejectedCategories, ...categoryExclusions].filter((category) => !lockedCategoryKeys.has(categoryKey(category)))); const approvedPool = approvedCategories.filter((category) => !excludedCategories.some((excluded) => categoryKey(excluded) === categoryKey(category))); return axios.post("/api/generate-session-candidates", { sessionId: `build-${activeRound.id}`, questionType: type, count, difficulty: difficultyOverride || difficulty, theme: themeOverride ?? theme, excludeUsed: true, avoidDuplicates: avoidDuplicatesOverride, includeImagePrompt: false, excludeCategories: excludedCategories, approvedCategories: approvedPool, rejectedCategories, lockedCategories: uniqueCategories(lockedCategoriesOverride), rejectedQuestions: [...rejectedQuestions], sessionContext: buildAiSessionContext() }, { timeout: 90000 }); };
  ...
  const normalizeGeneratedForRound = (candidates, round, fallbackType) => (Array.isArray(candidates) ? candidates : []).map((candidate, index) => applyDraftDefaults(normalizeQuestion({ ...candidate, id: `ai-${round.id}-${Date.now()}-${index}-${Math.random().toString(16).slice(2)}`, question_type: candidate.question_type || fallbackType, isGenerated: true }, fallbackType), round));
  ...
  const isUsableFullBuildCandidate = (question) => { const text = normalizeText(question?.question_text); const answer = normalizeText(question?.correct_answer); const category = normalizeText(question?.category); if (!text || !answer || !category || categoryKey(category) === "general") return false; if (text.length < 28 || answer.length > 80) return false; if (/^(what|which|who) (is|was|are|were) the (capital|largest|highest|longest|oldest) /i.test(text)) return false; return true; };
  ...
  const requestGeneratedForFullRound = async ({ round, type, count, excludedCategories, generatedQuestions, attempt = 1 }) => { const latestMemory = readQuestionMemory(); setQuestionMemory(latestMemory); const rejectedQuestions = new Set([...rejectedAi, ...memoryRejectedQuestionTexts(latestMemory), ...generatedQuestions.map((question) => question.question_text)]); const cleanExcluded = uniqueCategories([...rejectedCategories, ...recentSessionCategories, ...selectedSessionCategories, ...excludedCategories]); const approvedPool = approvedCategories.filter((category) => !cleanExcluded.some((excluded) => categoryKey(excluded) === categoryKey(category))); const fullTheme = [theme, `Build this as part of a full trivia session for ${sessionName || "this show"}.`, `Current target round: ${round.name}.`, `Required question type for this request: ${questionTypeLabel(type)}.`, round.description ? `Round category/direction: ${round.description}` : "", round.description ? "Treat the round category/direction as the primary instruction. The category field and every question must clearly fit that direction." : "", "Quality bar for full-session building: write host-ready bar trivia, not generic database trivia. Favor recognizable US-friendly subjects with fresh angles, satisfying clue paths, and broad reusable categories. Avoid bland textbook facts, random foreign-only facts, capital/largest/first list questions, and categories that feel like one-off answer labels.", "Generate fresh, reusable categories for this round. Do not recycle categories from recent sessions or categories already selected for this build. If the approved category pool gets too narrow, create adjacent categories with the same host vibe.", attempt > 1 ? `Retry ${attempt}: the previous batch did not fill the round. Send stronger, more concrete, more playable candidates and avoid every rejected or already generated angle.` : ""].filter(Boolean).join("\n"); return axios.post("/api/generate-session-candidates", { sessionId: `full-${round.id}`, questionType: type, count, difficulty, theme: fullTheme, excludeUsed: true, avoidDuplicates: true, includeImagePrompt: false, excludeCategories: cleanExcluded, approvedCategories: approvedPool, rejectedCategories, rejectedQuestions: [...rejectedQuestions], sessionContext: buildAiSessionContext() }, { timeout: 120000 }); };
  const collectFullBuildCandidates = async ({ round, type, needed, generatedCategories, generatedQuestions }) => { const accepted = []; for (let attempt = 1; accepted.length < needed && attempt <= 3; attempt += 1) { const missing = needed - accepted.length; const requestCount = Math.min(20, Math.max(missing + 3, Math.ceil(missing * 1.8))); const { data } = await requestGeneratedForFullRound({ round, type, count: requestCount, excludedCategories: [...generatedCategories, ...accepted.map((question) => question.category)], generatedQuestions: [...generatedQuestions, ...accepted], attempt }); const candidates = uniqueGeneratedQuestions(normalizeGeneratedForRound(data?.candidates, round, type), [...generatedQuestions, ...accepted]).filter(isUsableFullBuildCandidate).slice(0, missing); accepted.push(...candidates); } return accepted.slice(0, needed); };
  ...
  const handleBuildFullSession = async () => {
    ...
    const sourceRounds = shouldApplyTemplateShell ? normalizeRounds(makeTemplateBuildDraft(template, venues.find((venue) => venue.id === selectedVenueId) || venues[0] || null).rounds) : safeRounds;
    ...
    try {
      ...
      for (const round of sourceRounds) {
        ...
        for (const [type, typeCount] of Object.entries(plan)) {
          ...
          const generated = await collectFullBuildCandidates({ round, type, needed: typeCount, generatedCategories, generatedQuestions });
        ...
        if (remaining > 0) {
          const fallbackTypes = round.questionType && round.questionType !== "mixed" ? [round.questionType] : fullSessionFallbackTypes;
          for (const type of fallbackTypes) {
            const missing = needed - generatedByRound[round.id].length;
            if (missing <= 0) break;
            const generated = await collectFullBuildCandidates({ round, type, needed: missing, generatedCategories, generatedQuestions });
            generatedByRound[round.id].push(...generated);
            generatedQuestions.push(...generated);
            generatedCategories.push(...generated.map((question) => question.category));
          }
      ...
      if (!generatedQuestions.length) return toast.info("This build is already filled to the selected template count");
      ...
      if (shortages.length) toast.warning(`Built ${generatedQuestions.length} questions, but ${shortages.join(", ")} after retries`);
      else toast.success(`Built ${generatedQuestions.length} question${generatedQuestions.length === 1 ? "" : "s"} across the session`);
    } catch (error) {
