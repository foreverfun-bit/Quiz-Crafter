...
const HostTools = () => {
  ...
  useEffect(() => {
    ...
    setAttendance({
      barHeadcount: saved.barHeadcount ?? "",
      teamHeadcounts: saved.teamHeadcounts && typeof saved.teamHeadcounts === "object" ? saved.teamHeadcounts : {},
    });
...
const HostReviewWorkspace = ({ selectedSession, analytics, questionFeedback, categoryRows, playerIdeas, exportResults, attendance, setAttendance }) => {
  ...
  const totalPlaying = teams.reduce((sum, team) => sum + (Number(teamHeadcounts[team.id]) || 0), 0);
  const headcountsEntered = teams.some((team) => Number(teamHeadcounts[team.id]) > 0);
